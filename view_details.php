...
$stmt = $conn->prepare("SELECT title, description, category, type, status, admin_response, created_at, updated_at 
                        FROM feedback 
...
if (!$feedback) {
    logEvent($conn, $user_id, 'UNAUTHORIZED_ACCESS', "Student attempted to view feedback_id: {$feedback_id}");
    header("Location: my_feedback.php");
    exit();
}
...
logEvent($conn, $user_id, 'FEEDBACK_DETAIL_VIEWED', "Student viewed feedback_id: {$feedback_id}");
?>
...
<head>
    ...
    <style>
        ...
        .InProgress { background: #cce5ff; color: #004085; }
